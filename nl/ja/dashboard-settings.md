...
データ・セットが比較的小さくても、複雑な照会を実行する必要がある場合は、ストレージ・ノードはデフォルト・サイズの 5 GB のままにして、エンジン・ノードのメモリーを拡張すると、照会のパフォーマンスを向上させることができます。 Janus Graph Engine ノードにメモリーを追加するには、サポートにお問い合わせください。
